...
class OperationPlanService:
    ...
    def create_plan(self, *, request: OperationPlanCreateRequest, current_user: User) -> OperationPlanResponse:
        ...
        if request.operation_type == "RENAME_FILES":
            raise HTTPException(
                status_code=400,
                detail="RENAME_FILES plan must be generated by the controlled rename suggestion tool",
            )
    ...
    def confirm_plan(
        ...
        *,
        ...
        self.db.commit()
        self.db.refresh(plan)
        return OperationConfirmResponse(id=plan.id, status=plan.status, changeset_id=None, result={})
