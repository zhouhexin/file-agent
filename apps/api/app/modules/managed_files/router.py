...
@router.get("/api/managed-files/preview", response_class=FileResponse)
def preview_managed_file(
    root_key: str,
    relative_path: str,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
) -> FileResponse:
    """通过逻辑 root 和相对路径返回受管文件内容，供前端复用 Blob 预览流程。"""

    return ManagedFileService(db).get_preview_response(
        current_user=current_user,
        root_key=root_key,
