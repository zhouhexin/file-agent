    ...
    def classify(
        ...
        *,
        ...
        try:
            ...
            if full_text_primary_id and full_text_primary_id != summary_primary_id:
                # 分类摘要只用于压缩候选文本，不能在遗漏关键业务词后覆盖完整正文。
                # 发生 Top-1 冲突时统一采用全文候选，同时保留冲突快照供审计。
                base_categories = full_text_categories
            base_categories = [
                {
                    **category,
                    "summary_fulltext_agreement": summary_fulltext_agreement,
                    "candidate_scores": {
...
def is_managed_source_title_review_package(source_context: str) -> bool:
    """判断受管源是否位于已确认的人事处职称评定材料包。"""
    ...
    if len(parts) < 3 or ".." in parts:
        return False
    folded = [part.casefold() for part in parts[:-1]]
    return any(
        folded[index : index + 2] == ["人事处", "职称评定"]
        for index in range(max(0, len(folded) - 1))
    ) or folded[0] == "职称评定"


def _managed_source_title_review_package_category(
    *,
    source_context: str,
    taxonomy_key: str,
    taxonomy_version: str,
) -> dict[str, Any] | None:
    """把职称材料包统一归入学校职称业务，并保留源容器审计证据。"""

    if not is_managed_source_title_review_package(source_context):
        return None
    container = str(source_context).replace("\\", "/").rsplit("/", 1)[0]
    evidence = {
        "type": "managed_source_container",
        ...
        "quote": container,
        "signals": ["人事处/职称评定材料包"],
        "source": _MANAGED_SOURCE_TITLE_REVIEW_PACKAGE_SOURCE,
    }
    return {
        "name": "学校/人事师资/职称",
        "category_id": _TITLE_REVIEW_CATEGORY_ID,
        "category_path": ["学校", "人事师资", "职称"],
        "confidence": 1.0,
        "status": "SUGGESTED",
        "source": _MANAGED_SOURCE_TITLE_REVIEW_PACKAGE_SOURCE,
        "evidence": ["人事处/职称评定材料包"],
        "evidence_items": [evidence],
        "matched_title_signals": [],
        "matched_content_signals": [],
        "negative_signals": [],
