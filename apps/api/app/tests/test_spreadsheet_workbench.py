...
def test_workbench_profile_consumes_persistent_xlsx_for_original_xls(tmp_path: Path) -> None:
    """表格工作台只消费已解析的持久化 XLSX，同时对外保留原始 XLS 类型。"""

    path = tmp_path / "persistent-derivative.xlsx"
    workbook = openpyxl.Workbook()
    worksheet = workbook.active
    worksheet.title = "成果"
    worksheet.append(["教师", "资助金额"])
