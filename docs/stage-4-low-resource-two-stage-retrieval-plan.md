...
低耗模式只复用 PostgreSQL、阶段三已有 GIN/`pg_trgm` 索引和应用层 Jieba。SQLite 只用于确定性
单元测试，不作为生产检索能力承诺。
...
- `L4`：当前用户 default workspace 中状态为 `ACTIVE` 的工作副本。
...
- `final_filename` 使用 `pg_trgm` GIN 索引，处理短语和少量拼写差异。
- `search_text` 由应用层 Jieba 产生稳定词项，不能保存任意 LLM 输出。
- `source_fingerprint` 覆盖版本、最终文件名、摘要和分类版本，用于判断是否需要重建。
...
第一阶段结果只携带稳定业务 ID、受控概览和分项信号，不返回正文。
...
- PostgreSQL 使用 `search_vector` GIN 为主，`pg_trgm` 只做短语和拼写差异补充。
...
不得包含 Skill、Tool、AgentRun、内部队列、原文件路径、上传原文件名、SQL 分数、`search_text`、
embedding 或完整正文。
...
逐文件查询。应用层只保留候选投影和少量 Evidence 预览，不能保留全文。
...
1. 新增 `DocumentSearchProfile` ORM 模型和 Alembic migration。
2. 实现投影 upsert、失效、分页 backfill 和 fingerprint 幂等判断。
3. 接入首次整理、内容新版本、确认后重命名、分类更新、回收站和恢复事件。
4. 添加 PostgreSQL GIN/`pg_trgm` 索引与 SQLite 测试降级。

退出条件：重复构建不产生重复记录，重命名不重建 Chunk，回收站文件不再被召回。
...
2. 同时召回最终文件名、全部有效分类、年份、实体、关键词和摘要。
3. 添加文件名 trigram 召回和安全绑定参数。
4. 保留 SQLite deterministic fake，用于无 PostgreSQL 的业务测试。

退出条件：旧文件不会因“最近 500 条”限制被漏掉，应用内存与工作区总文件数不线性增长。
...
1. 整理后文件名精确命中。
2. 文件名轻微差异通过 `pg_trgm` 命中。
3. 主分类和第二分类都可召回。
4. 显式年份和“去年”可召回正确文件。
5. 实体、单位、关键词和摘要命中。
6. 摘要没有主题词、原文 Chunk 有主题词时仍可召回。
7. PDF 返回真实页码，XLS/XLSX 返回 Sheet 和单元格范围。
8. L0/L1/L4 同等相关时顺序正确。
9. 明确“这些附件”不会扩大到工作区。
10. 全局查询不会因会话里存在旧附件而错误限制为 L1。
11. 不能召回其他用户、回收站、隐藏临时文件、旧版本或失败索引。
12. 重命名后新名称可搜，Chunk ID 不变。
13. 内容新版本只召回当前版本。
14. projection backfill 和重复 upsert 幂等。
15. Chunk 超时、索引缺失和 embedding disabled 可正确降级。
16. SQL/tsquery 特殊字符只作为数据处理。
17. 候选、查询词、Chunk 和预览长度硬上限有效。
18. 查询数量固定，无逐文件 N+1。
19. AgentGraphState、日志和普通用户回执不含正文、`search_text`、embedding、路径、Skill 或 Tool。
20. 普通消息入口和 `/api/search` 使用相同所有权边界和排序结果。
21. SQLite deterministic 测试和 PostgreSQL 集成测试都不调用互联网、LLM 或 embedding。
22. Windows 路径、事件循环和测试临时目录不参与搜索逻辑，测试可跨平台完成。

LLM 和 embedding 相关测试必须使用 deterministic fake，并额外断言低耗默认配置下 fake provider 也没有
被调用。
