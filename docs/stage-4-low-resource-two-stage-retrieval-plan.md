...
- `L4`：唯一系统共享工作目录中、所有普通用户可见且状态为 `ACTIVE` 的工作副本。default workspace
...
- `WorkingCopy.workspace_id` 属于唯一 `SYSTEM_SHARED` 工作区。
- `WorkingCopy.status = ACTIVE`。
- 结果版本等于 `WorkingCopy.current_version_id`。
- 对应 `DocumentIndexRun.status = COMPLETED`。

默认排除原始归档、隐藏临时文件、回收站文件和旧内容版本。L0/L1 会话附件仍按当前用户隔离，L4
可以召回唯一共享工作目录中的活动文件，但不得携带其他用户的会话、上传来源、反馈或确认记录。即使
索引投影数据陈旧，最终共享范围与活动版本校验也不能省略。
...
退出条件：范围确定、个人会话与附件上下文隔离、共享活动文件可访问、相对时间解析可重复验证。
...
13. 可以召回共享 `ACTIVE` 文件，但不能召回回收站、隐藏临时文件、旧版本或失败索引，也不能泄漏
    其他用户的个人会话、上传来源和反馈。
