...
修订：v3，按 2026-09-11 最新要求恢复“学校/学院—部门—发文/其他”受控收纳，并优先实现材料包用途继承和常见材料结构规则；仍不产生“待复核”“待分类”分类、流程或队列，明确更正主分类、明确移动直接执行且不二次确认。配套实施依据：[开发实施文档](2026-09-10-classification-optimization-development-spec.md)。
...
无法可靠细分到具体业务叶类时，优先采用受控范围收纳：正文能可靠确定学校/学院范围及部门业务域的，进入“学校/学院—部门—发文/其他”；存在可定位正式文号进入“发文”，否则进入“其他”。此外，文件名中的严格正式文号若包含可唯一映射到受控部门的发文机关前缀，也可直接确定该部门“发文”；它不能证明更具体业务。组织范围或部门也无法可靠确定时才使用稳定 `system.other`，显示路径与物理目录均为 `其他`。具体业务父类有充分依据时仍优先于 fallback。
...
材料包规则首批按通用结构机制支持人才申报、聘任考核、教师招聘、职称、专业认证、专业/培养/课程建设和科研项目材料。不得把整个 `人事处`、`办公`、`外来应聘` 根目录作为一个包。

每个包必须有稳定 `package_id`、受控来源索引 ID、不可变成员清单及哈希、用途类别、规则版本、授权来源、识别依据和允许的收纳维度。当前自动识别从当前叶目录向上寻找最近的具体材料容器，可包含其“照片/附件”等子目录，但遇到年份或 `人事处/办公/外来应聘` 等宽泛集合根立即停止；至少一个成员必须以原始正文中的“精确题名＋成组字段”形成唯一结构锚点，包内出现不同业务锚点或存在未完成分析的成员时不得继承。单文件正文的业务证据与包用途依据分开存储，目录位置本身不能成为用途证据。
...
以下节点最初按 `2026-09-v10` 提案并已进入配置；组织范围词和优先级调整顺延为 `2026-09-v11`，文档主旨证据排序和业务词表补全为 `2026-09-v12`，恢复范围 fallback、材料包继承和常见结构规则为当前 `2026-09-v13`。

| 候选节点 | 建议 ID | 依据与约束 |
| --- | --- | --- |
| 全局其他 | `system.other` | 组织范围或部门均无法可靠确定时的最终兜底，显示路径及 organization_path 均为 `["其他"]`；不创建 `system.unclassified` |
...
正文有可靠业务证据但缺乏叶类证据 → 选择已配置路径的业务父类；具体业务不足但学校/学院及部门均可靠 → 有正式文号进入该部门 `.issued`，否则进入该部门 `.other`；组织范围或部门不可靠 → `system.other`。原件风险检查不通过时不得借兜底强制发布。

`.issued/.other` 作为不可召回的 `FALLBACK` 节点恢复自动收纳能力。一般情况必须先由正文确定组织范围和部门；窄范围例外是文件名严格文号的机关前缀可唯一映射到受控部门，此时可确认该部门“发文”，但不能确认具体业务。近似编号、普通机构词或多个部门竞争时关闭式拒绝。无法形成该范围链时使用 `system.other`。
...
## 10. 2026-09-11 组织范围关键词与优先级补充

根据 `E:\workdata` 的实际目录/文件用词，taxonomy `2026-09-v11` 扩展组织范围词，但这些词只判定“学校/学院”根，不直接生成具体业务分类。学院范围新增计算机科学与技术、计算机技术、软件工程、网络工程、网络安全、信息安全、网络空间安全、物联网工程、人工智能、数据科学与大数据技术、智能科学与技术、数字媒体技术，以及计算机系、软件系、网络系、网络安全所、计算机实验室等表达；学校范围新增教务处、人事处、信息化处、发展规划处、研究生院等在盘点中实际出现的校级部门。

冲突按组织身份和语句角色处理：完整表达“西安理工大学计算机科学与工程学院”判学院，校名不覆盖其中的学院身份；“计算机学院报送教务处”中教务处是接收方，仍判学院；“教务处下发/印发/部署/通知各学院”中教务处是发布主体，判学校。职称表单正文中的“现从事学科：计算机科学与技术”等对象属性不作为学院形成单位，避免既有校级职称材料退化。

组织范围未知不等于检测到相反组织证据。具备可定位正文引用、业务分达到当前门槛且不存在负向冲突的业务候选可以参与 PRIMARY；只有明确检测到学校/学院相反范围、业务竞争未拉开差距、证据矛盾或正文不足时才回退 `system.other`。新增范围词不会降低业务证据要求，也不会单靠目录名、文件名或一个组织词制造具体业务类。

## 11. 2026-09-11 文档主旨优先与具体分类扩展

真实回放显示，“其他”文件多数并非解析失败，而是业务候选低于 0.45，或长文背景词制造跨分支竞争。`2026-09-v12` 因此采用分层证据权重：文件名/标题中的专属业务短语最高，正文前 2000 字或 Excel 首部次之，其余正文只提供较低分。组织范围仍独立计算；标题中的“人才引进、专业建设、合同期考核”等主旨不得被正文中的“信息化、审计、专业认证、科研任务”等背景字段压过。

一般业务分门槛保持 0.45；证据支持度至少 0.55 时可按 0.35 进入竞争；标题主题分至少 0.16 且证据支持度至少 0.35 时门槛为 0.28；正文开头主题分至少 0.16 且证据支持度至少 0.35 时门槛为 0.30。单个泛词没有足够独立证据时不享受降低门槛。跨业务分支第一名不再要求领先 0.10；通过证据门槛后，只要最终排序分严格高于第二名就作为唯一 PRIMARY。完全同分、证据矛盾或真正多主题汇编仍归“其他”。该调整用于提高具体叶子分类覆盖率，不允许仅靠原目录、扩展名或组织词落位。

主类排序额外加入受限位置加成：标题主题最多 0.15、正文开头主题最多 0.04。加成只能作用于已被 taxonomy 召回且已有正文定位证据的业务候选，用于阻止“师资引进申请报告”被正文背景中的规划、科研等高频词覆盖。

同步升级确定性规则策略至 `workdata-v2`，补齐人才推荐申报、人才引进计划、师资引进申请报告、专业建设、培养方案、课程体系和合同期考核等词组，并收紧信息化、审计、专业认证规则中的泛化动作词。无正文图片、无任何业务候选、明确组织冲突及真正多主题汇编继续进入唯一 `system.other`。

最终只读校准从当前 328 个 `system.other` 活动工作副本中按路径等距抽取 66 份，限制候选输入为前 15000 字但仍从真实页面定位证据：25 份可进入人才工作、教学、聘任考核、科研等具体分类，41 份保留其他；保留项中 `LOW_QUALITY=34`、`NO_BUSINESS_EVIDENCE=6`、`AMBIGUOUS_PRIMARY=1`。`20220318师资引进申请报告-李院.docx` 的定向回放已由发展规划纠正为学院人才工作。该比例只用于本轮规则回归，不作为概率校准结论。

## 12. 2026-09-12 人才、人事与材料包修订

分类器 v19 和规则策略 `workdata-v3` 落实以下增量：

- 师资引进申请报告归人才工作；人才项目“推荐意见”结合高层次人才、青年拔尖或领军人才用途提高人才工作权重；
- 教师职务聘期任务书按精确题名和字段结构归师资招聘；西安理工大学教职工离职审批表归学校人事师资；
- 学校/学院范围与业务候选镜像不一致时，切换到同一业务的受控镜像节点；
- 两个合格跨分支候选差值不足 0.10 时，以最终排序分最高者为主类，完全同分才保留歧义；
- 扩展真实人才申报书锚点，并让版本刷新重新尝试材料包识别，解决旧锚点漏建后附件不能继承的问题。

真实正文回放确认上述五个代表文件均得到目标主类；“黑新宏”真实叶目录可形成 3 成员人才用途包。详细规则、回放结果和上线刷新边界见开发实施文档 §18。

## 13. 2026-09-12 v20 后续校准修复

真实抽查进一步发现：用途包在分类层已通过，但落位层把无页码的清单证据误判为缺少证据；摘要阶段产生的根级其他会挡住完整正文的部门信息，且部门父节点未进入业务 Top-N 时也无法兜底。v20 通过持久化包成员/版本/哈希再验证和独立的完整正文部门识别修复这两条链路。

规则策略升级 workdata-v4，补充 CJ、青年教师奖等人才汇总表，校招共用引才用才，合同期满考核，以及以聘任时间/聘期/任职状态为核心的人员表结构；人才用途锚点随之扩展。强结构候选在候选截断前保留，避免先按组织排序将其挤出。具体接口、证据边界、验收用例和上线范围见开发实施文档 §19。仍复用 v13 既有目录，保留多标签及其他兜底，不新增二次确认。
