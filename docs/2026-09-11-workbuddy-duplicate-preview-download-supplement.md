...
- 2026-09-13 用户新增要求将浏览器对比改为公开只读链接；下文旧版“必须登录”描述由开发规范第 14 节覆盖，重复决定和普通文件访问仍需登录。
...
## 10. 公开链接的当前产品边界（2026-09-13）

用户现要求在 WorkBuddy 点击重复对比链接后直接显示页面，不进行浏览器登录。链接仍由原有 MCP
导入会话产生，只包含固定候选身份与修订；浏览器和三个对比 GET 接口允许匿名读取当前有效 review
的两侧内容及下载，不放开 `duplicate_review_get`、`duplicate_decide` 或普通文件接口。
因此，在链接有效期内，任何网络可达且持有链接的人都能读取两侧完整文件；这是本次明确接受的公开范围，
不是“局域网自动鉴权”。页面仍不提供决定按钮，过期或版本变化后继续沿用原拒绝行为。
