...
`RENAME_FILES` and `RENAME_UPLOADED_FILES` cannot be created with this generic endpoint. They must be
generated by the controlled `generate-rename-suggestions` Tool. Uploaded-attachment plans contain only
`document_id`, before/after basenames, content hash and rename evidence; they never accept a storage path.
...
execute RENAME_FILES through the managed Native/F2 executor, or RENAME_UPLOADED_FILES through the temporary-storage executor
write confirmed-file-action ToolInvocation, ChangeSet, per-file success/failure, and final plan status
reject unsupported MOVE/DELETE/OVERWRITE operation types with 409 and keep the plan waiting
```
...
```json
{
  ...
  "result": {
    ...
    "items": [
      {
        "document_id": "document-uuid",
        "before_filename": "扫描件.txt",
        "after_filename": "2026_春季学生活动总结.txt",
        "status": "COMPLETED"
      }
    ]
  }
