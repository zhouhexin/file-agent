...
- 不重命名目录。
- 不覆盖同名目标文件。
- 不覆盖冲突文件；自动建议由后端按受控版本策略生成 `_第二版`、`_第三版`，用户手工指定名称仍需处理冲突。
- 不执行移动、删除或任意路径写入。
...
- 年份或标题缺失、字段冲突或证据不足时，状态进入 `NEEDS_REVIEW`。
- `NEEDS_REVIEW` 项从当前批次 OperationPlan 中跳过，并在回执中逐项说明原因。
...
用户消息
-> 后端解析受管文件范围
-> managed-file-read-document 创建或复用安全快照和 document_pages
...
- `NEEDS_REVIEW` 项从可执行批次中跳过，不阻止其他 READY 项确认执行。
- 展示成功、失败、跳过和未修改原件说明。
