...
- 状态：第一阶段已实现；初始化策略将按源文件预分析与工作副本按需物化方案调整
- 编写日期：2026-07-20
- 修订日期：2026-08-13
- 适用范围：受管原始目录中的文件导入、同步，以及工作副本的增删改查
- 架构边界：继续使用 Agent Runtime、白名单 Tool、OperationPlan、ChangeSet 和 StorageService

> 2026-08-13 调整：普通受管原始目录不再以“扫描后立即为全部原始文件创建工作副本”为目标。
> 新目标是先异步建立原始文件摘要、关键词、实体、正文分块和表格结构，查询时同时覆盖原始文件与
> 工作副本，文件被实际读取或操作后再物化工作副本。具体任务、数据模型、检索去重和迁移顺序以
> `docs/managed-source-index-lazy-working-copy-implementation-plan.md` 为准。本文中与该调整冲突的
> “启动时全量导入”“周期同步自动补齐全部工作副本”描述均按新方案解释；三层目录、原始文件只读、
> OperationPlan、ChangeSet 和回收站安全边界保持不变。
...
- 批量导入必须分页处理并记录进度。
- 同步阶段不同目录或来源中的同名文件必须全部正常导入，不得写入用户可见的`待整理`或`待确认`目录。完整物理目标被数据库外的不同内容占用时，使用`.internal/import-collisions/<managed_file_id>/<filename>`隐藏隔离路径保护未知内容；同名歧义只在上传、查询或实际使用相关文件时提示用户选择。
