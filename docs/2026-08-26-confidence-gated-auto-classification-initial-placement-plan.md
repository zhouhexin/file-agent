...
2026-09-05 根据 `E:/workdata` 的真实目录结构补充首次落位规则。外部受管文件通过主分类门槛后，
不能一律把完整旧部门目录追加到 taxonomy 路径，也不能一律丢弃材料包内部结构：

- 普通部门、年份和历史人工分类目录默认扁平化；完整来源继续只保存在
  `ManagedFile.relative_path` 和工作副本追溯关系中。
- 人员、项目、活动、审计、评估、认证和凭证材料包选择性保留包内相对路径。
- 顶层集合锚点（例如“外来应聘”“学院照片”）不重复进入 taxonomy 路径；年度、人员、活动等
  锚点以下路径继续保留。
- 部门下的具体项目（例如某次审计或专项治理）只移除部门名，保留项目名和包内路径。
- `人事处/职称评定` 识别为受管职称材料包：移除这两个集合锚点，但保留年度、系列、评审类型、
  个人姓名及其内部相对路径；该包内文件的主业务分类固定为 `学校/人事师资/职称`，文件自身的论文、
  教学或学科内容只作为次级检索事实，不得覆盖材料包用途。
- 上传归档的内部 `uploads/<yyyy>/<mm>/...` 路径始终不进入用户可见分类目录。
- 已经 `ACTIVE` 的历史工作副本不因策略上线而静默移动；如需整理，仍必须生成并确认
  `MOVE_WORKING_COPIES` OperationPlan。

当前阶段按用户要求将目录清单硬编码在
`apps/api/app/modules/managed_files/source_path_policy.py`，后续再演进为可审计配置。扫描阶段同时过滤
已确认的安装包、驱动、测试目录和系统垃圾文件；过滤只影响索引与后续物化，不删除或修改受管原件。
策略上线前已经登记的非业务文件标记为 `IGNORED`，不得误报为 `MISSING`；如果该原件之后确实消失，
下一轮完整扫描再转为 `MISSING`。既有 `ACTIVE` 工作副本仍不由扫描器静默删除或停用。
...
- 受管源分析与上传分类统一使用已加载的完整正文作为兜底证据文本；分类实现版本升级为 `v8`，
  使证据兜底逻辑变化能够自动失效旧分类缓存并重新提交源侧分类刷新任务。
- TXT 文件在上传归档和受管目录物化同步时保留原文件名，状态记为 `NO_CHANGE`；TXT 仍继续执行
  正文解析、分类、索引和首次分类目录落位。用户后续明确发起的重命名请求仍走既有 OperationPlan。
...
## 31. 2026-09-05 职称材料包、全文主分类与首次命名纠偏

本次修复以受控正文和已确认的受管源容器为准，解决职称评定表被错误归到学科/科研，以及“专家鉴定
意见表”“评审简表”被错误命名为个人简历的问题：

1. 分类主题摘要与完整正文 Top-1 不一致时，最终主分类采用完整正文候选；冲突仍写入
   `summary_fulltext_agreement=false` 供审计。当前继续使用 `auto-placement-top1-test-v1`，不恢复最低
   置信度或 Top-1/Top-2 候选分差门禁。
2. taxonomy 增加“专家鉴定意见表、教师职务任职资格评审表、职务评审简表、评审简表”等职称强信号，
   版本升级为 `2026-09-v9`，分类器实现版本升级为 `v14`，使旧源侧分类缓存自动失效。
3. `人事处/职称评定` 是已确认的受管源业务材料包。同步时统一使用
   `学校/人事师资/职称` 主分类，并保留年度、系列、评审类型、个人姓名和包内相对路径；受管原件不变。
4. 自动命名为“个人简历”只在受管源导入物化阶段启用；普通批量上传不套用该启发式规则。受管源文件
   只有标题或原文件名明确含“简历/CV”，或者处于已确认招聘材料包且正文具有完整简历结构时，才允许
   自动采用个人简历名称。
5. 专家鉴定意见表、任职资格评审表和评审简表属于受保护表单。即使正文含“姓名、性别、学历、工作
   经历”等字段，也不得据此重命名为个人简历；OCR 标题不可靠时保留原文件名，避免将识别错误发布为
   最终名称。

这些规则只影响新导入或清理后重新物化的工作副本。已经 `ACTIVE` 的错误工作副本不会后台静默改名或
移动，须按既有重置/重新物化流程或确认后的 OperationPlan 更正。
