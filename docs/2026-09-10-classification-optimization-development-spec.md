...
版本：v3；日期：2026-09-12；状态：开发验证中。本文是开发与验收契约；第 19 节记录本轮材料包落位、部门兜底和人事规则修复。

上位方案：[分类规则优化方案 v3](2026-09-10-workdata-classification-rule-optimization-plan.md)。本轮用户明确要求恢复“学校/学院—部门—发文/其他”，优先实现材料包用途继承并为常见材料建立结构规则；仍不出现分类“待复核/待分类”，明确更正和明确移动不二次确认。与旧规范冲突的地方，执行本轮要求。
...
| 编号 | 必须成立的结果 |
| --- | --- |
| INV-01 | 每个纳入新版策略且发布成功的 ACTIVE 工作副本，其当前版本恰好有一个有效 PRIMARY；可以为业务父类、业务叶类、已授权用途类、范围 fallback 或 `system.other` |
...
新自动兜底分两级：正文能可靠确定学校/学院及部门时使用该部门 `.issued/.other`，前者要求可定位正式文号；文件名严格文号中的机关前缀若能唯一映射到受控部门，也可直接使用该部门 `.issued`。组织范围或部门也无法可靠确定时使用 `system.other`。`system.other` 的显示路径、organization_path 均为 `["其他"]`。

范围 `.other/.issued` 保持稳定 ID，编译为不可普通召回但可落位的 `FALLBACK` 节点；它们只能由已验证的组织范围、部门及文号规则生成，不能通过目录名或单个机构词捏造范围。
...
fallback 配置保留 `target_category_id=system.other` 作为最终兜底，同时列出允许物化“发文/其他”的部门业务父节点。物化节点必须是 `FALLBACK`、`recall_enabled=false`、`primary_enabled=true`；不得添加“未分类/待分类”召回别名。

校验必须覆盖 ID 唯一、节点角色、正反映射、Unicode NFC 与大小写归一化路径冲突、Windows 保留名、路径段长度、嵌套分类边界。系统根路径不可与任何来源归档根重叠。规范化只用于比较，不静默重写用户历史文件名。

首版业务新增固定采用方案 §5.2 的信息化双节点、学院专业认证/教学评估、学校本科评估、办公参考资料节点；分别添加真实正文正反样本并更新构建输入。节点首发为 `2026-09-v10`，组织范围词和优先级调整为 `2026-09-v11`，文档主旨证据排序和业务词表补全为 `2026-09-v12`，范围 fallback、用途继承和结构规则为当前 `2026-09-v13`。
...
6. 无充分具体业务主类，但组织范围和部门均可靠时，选择该部门 `.issued/.other`：仅有可定位正式文号进入 `.issued`，其余进入 `.other`，outcome=CLASSIFIED、basis=SCOPED_FALLBACK。
7. 组织范围或部门也不可靠时返回 `system.other`、outcome=OTHER、basis=FALLBACK；原因为 NO_BUSINESS_EVIDENCE / AMBIGUOUS_PRIMARY / UNKNOWN_SCOPE / LOW_QUALITY / PARSE_UNAVAILABLE 等。
...
首批结构锚点覆盖人才申报、聘任考核、教师招聘、职称、专业认证、专业/培养/课程建设和科研项目材料。生产入口在同一具体叶级容器成员全部完成分析后，根据原始正文的精确题名和成组字段查找唯一锚点，冻结成员版本与 SHA 后创建用途包；若出现竞争用途或成员不完整则不创建。目录名只界定成员候选，不能决定业务用途。
...
对历史 `.issued/.other`，查询继续解析原稳定 ID；当前 fallback_policy 中重新启用的部门节点可由受控规则选择，未启用节点只读兼容。保留旧人工来源，不默默结束。历史 NEEDS_REVIEW 的建议记录不直接当 PRIMARY，也不复制写成 CONFIRMED。
...
- [ ] taxonomy/规则构建可重现；范围 fallback 与全局 OTHER 分层兜底；所有 fallback 退出普通召回但允许受控落位。
...
- [ ] 分类树、分页、计数、聊天与批次回执区分范围 fallback 和全局 OTHER，无分类复核队列；非分类冲突/失败仍真实可见。
...
## 15. 2026-09-11 taxonomy v11 组织范围实现约束

`2026-09-v11` 在 matcher 中增加组织范围优先级层，输入仅限受控的文件名、标题和完整正文。计算机学院全称及计算机相关专业/系所词优先判学院；校级部门名称判学校，但部门作为“提交、报送、上报、送交、反馈”的接收方时不得覆盖学院主体，作为“印发、发布、下发、部署、决定、要求、通知”的发布主体时判学校。校名和学院全称组合必须判学院。

实现必须保留三条边界：组织词只产生 scope 分和组织审计信号，不替代业务节点证据；职称表单中的学科/研究方向字段不得被当作形成单位；`organization_scope=null` 只表示未识别，不等价于相反范围冲突。对应单元测试至少覆盖校名+学院全称、五类计算机专业词、学院报送校级部门、校级部门发布以及校级职称表单不退化。

## 16. 2026-09-11 taxonomy v12 文档主旨优先实现约束

`2026-09-v12` 将业务候选评分按“文件名/标题、正文前 2000 字或表格首部、其余正文”分层。标题中的专属业务短语权重最高，正文开头次之，长正文其余位置只保留低权重召回和可定位引文。学校/学院组织词继续只影响 scope，不得混入 business 分。负向词只有在标题或正文开头形成明确冲突时阻止落位，其余位置只扣分。

主类仍必须具有来自原正文的页码或 Sheet 定位证据。一般候选业务分门槛维持 0.45；证据支持度至少 0.55 时门槛为 0.35，标题主题分至少 0.16 且证据支持度至少 0.35 时门槛为 0.28，正文开头主题分至少 0.16 且证据支持度至少 0.35 时门槛为 0.30。单个泛词形成的低证据候选仍使用 0.45，不因本次放宽自动落位。跨业务分支不再因差值小于 0.10 自动退回“其他”；只要第一名最终排序分严格更高就直接作为唯一 PRIMARY，完全同分、证据矛盾或真正多主题汇编才视为确实歧义。图片无正文、没有业务候选、明确组织冲突或真正多主题汇编仍归 `system.other`。

候选进入主类选择后，以业务分为基础，标题主题最多提供 0.15 排序加成，正文开头主题最多提供 0.04 排序加成；该加成只改变已有正文业务候选的排序，不新增候选、不代替证据门槛。这样可以让“师资引进申请报告”等标题主旨压过正文中的发展规划、科研任务等背景词。

规则策略升级为 `workdata-v2`：信息化、专业认证和审计组合规则删除可由同一个泛词重复满足的条件；学院与学校人才工作补充“高层次人才引进计划、人才推荐申报、人才汇总表、师资引进申请报告”等真实业务短语；学校本科教学补充“专业建设、培养方案、课程体系、一流专业”。工作副本复用源分类时必须同步复制 `classification_outcome`、`classification_quality`、`selection_basis`、`reason_codes` 和主类输入指纹，不得丢失真实原因。

2026-09-11 对当前 328 个 `system.other` 活动工作副本按路径等距抽取 66 份、使用前 15000 字正文做只读回放：25 份可转入具体分类，41 份继续归其他；剩余原因为 `LOW_QUALITY=34`、`NO_BUSINESS_EVIDENCE=6`、`AMBIGUOUS_PRIMARY=1`，其中 9 份同时缺少明确组织范围。该结果说明门槛调整能回收明确业务文档，但不会把学历证书、身份证明、照片或独立附件仅凭文件名强行归入人才包。定向回放已确认 `20220318师资引进申请报告-李院.docx` 选择 `college.hr.talent-work`。

## 17. 2026-09-11 taxonomy v13 范围兜底、材料包与结构规则

`2026-09-v13` 恢复“学校/学院—部门—发文/其他”作为受控落位节点。加载器只对 `fallback_policy.department_category_ids` 中登记的业务父节点物化 `.issued/.other`，节点固定为 `FALLBACK`、`recall_enabled=false`、`primary_enabled=true`。普通 matcher、语义召回和业务竞争不得直接召回这些节点；具体业务选择失败后，fallback 构造器才可依据正文中已命中的部门证据与组织范围创建范围候选。文件名命中严格正式文号格式且机关前缀唯一映射到一个受控部门时，可将该不可变元数据作为窄范围例外生成部门 `.issued`；它只证明组织、部门和发文，不能证明具体业务叶类。近似编号或部门竞争时进入 `system.other`。

常见材料结构规则采用“精确题名＋独立字段组”双门槛，不采用“申请表、合同、任务书”等单个泛词。首批覆盖：高层次人才/人才计划申报、教师岗位聘用与聘期考核、教师招聘/试讲、职称申报与专家鉴定、专业认证、专业/人才培养/课程建设、科研项目申报与任务书。同一文件形成两个不同结构用途时不产出结构锚点。所有定位 quote 继续从 `document_pages.text_content` 原文生成。

受管目录生产入口在分类前调用 `ManagedPurposePackageInferenceService`。当前实现从文件所在叶目录开始向上寻找最近的具体材料容器，允许该容器包含“照片/附件”等嵌套目录，但遇到受管根、年份、`人事处`、`办公`、`外来应聘` 等宽泛容器立即停止；成员数限制为 2–50。除正在处理的成员外，其他成员必须已有当前 READY 修订、`document_version_id` 和 SHA-256。包内恰好存在一个结构用途时，服务冻结所有成员的 managed_file_id、document_version_id、SHA-256、taxonomy 版本和 manifest digest，写入不可变 `classification_purpose_packages`。最后完成的成员触发前序 READY 成员分类刷新，使附件继承同一用途；刷新作业携带 package_id 和 manifest digest 作为去重输入。出现成员不完整、多个用途或哈希/版本不匹配时关闭式退回单文件分类。

本轮新增回归要求：泛化申请表不形成结构用途；专业建设方案需题名及至少两组字段；冲突结构关闭继承；真实 SQLite 持久化流程验证唯一锚点包创建、成员冻结及现有快照复用。范围 fallback 继续覆盖学校/学院下“发文/其他”，且不得给已经成立的具体业务主类追加 fallback 辅助污染。

## 18. 2026-09-12 classifier v19 人才、人事、组织镜像与材料包补强

本轮不修改 taxonomy 节点树，继续使用 `2026-09-v13`；确定性规则策略升级为 `workdata-v3`，分类器实现身份升级为 `v19`，使既有受管文件的新鲜度检查能够识别本轮行为变化并复用已持久化正文重新分类。

主类竞争按已通过证据门槛的候选最终排序分确定。最终排序分由业务分、最多 0.15 的标题主题加成和最多 0.04 的正文开头加成组成；跨分支第一名与第二名即使相差不足 0.10，只要第一名严格更高也直接落位。完全同分且没有主旨优势时仍进入 `system.other`，不按 taxonomy 顺序任意裁决。

组织范围校正发生在业务候选过滤阶段：候选属于 `college.*`、正文组织范围却明确为学校时，若 taxonomy 存在同后缀 `school.*` 且该节点允许 PRIMARY，则复制原业务证据并切换到学校镜像；反向同理。该过程只切换组织根，不新增业务语义。结构表单的题名范围优先于表单内部“所在单位”等填报字段，因此“西安理工大学教职工离职审批表”即使正文出现“计算机学院”，仍归 `school.hr`。

`workdata-v3` 新增人才项目推荐意见组合规则：正文出现高层次人才特殊支持计划、三秦英才、青年拔尖人才、科技创新领军人才或人才项目申报，同时题名/正文出现推荐意见、推荐申报或推荐人选时，提高人才工作候选，不让个人科研履历抢占主类。常见材料结构新增或调整如下：

- “师资引进申请报告”由标题主旨和人才引进正文证据归学校/学院“人才工作”；
- “教师职务聘期任务书”在精确题名和聘期、受聘期间、工作任务等字段同时成立时归“师资招聘”；
- “西安理工大学教职工离职审批表”在精确题名和申请人、所在单位、人事处审核意见、辞职申请书等字段同时成立时归学校“人事师资”；
- Word 表格把“受聘期间”等字段按字符换行抽取时，结构规则允许在该字段内部跨换行匹配，但保存的引用仍截取原始页面文本。

人才材料包锚点补充“科技创新领军人才申报书”“青年拔尖人才申报书”“哲学社会科学和文化艺术人才申报书”等真实题名变体，仍要求至少两个独立首页字段组并拒绝冲突用途。受管源分类刷新在未携带包 ID 时也会重新调用 `ManagedPurposePackageInferenceService`；因此首次解析时因旧锚点词表漏建的现有材料，在 v19 刷新时可冻结不可变清单，随后为同包其他 READY 成员排队刷新。包成员、版本、SHA-256 和 manifest digest 校验保持不变。

真实正文只读回放已覆盖：

- `20220318师资引进申请报告-计算机学院-提交版.docx -> college.hr.talent-work`；
- `推荐意见-罗靖.docx -> college.hr.talent-work`；
- `西安理工大学教师职务聘期任务书（2022年）-杨志海.doc -> school.hr.faculty-recruitment`；
- `附件4：西安理工大学教职工离职审批表.docx -> school.hr`；
- `附件3：西安理工大学2024年拟引进高层次人才候选人统计表.xls -> school.hr.talent-work`。

真实“黑新宏”叶级目录在回滚事务中形成 `school.hr.talent-work` 用途包，冻结 3 个成员；验证事务已回滚，未在正式数据库保留测试包。本轮定向单元测试和分类/worker 回归通过后，部署时须运行正式刷新任务才会更新数据库关系与工作副本；仅修改源码不会主动搬动现有文件。

## 19. 2026-09-12 classifier v20 落位证据链与人事表单修复

继续使用 taxonomy `2026-09-v13` 的既有稳定节点，不新增目录或迁移。分类器升级 `v20`，规则策略升级 `workdata-v4`，使旧分类缓存可被识别为过期；原始解析、OCR 和索引事实可以复用。

### 19.1 材料包落位

已确认的缺陷是分类层 `VERIFIED_PACKAGE` 使用 `purpose_package_manifest` 引用，而自动落位只承认原文页码或 Sheet；实际抽查中 17 个包成员因此以 `EVIDENCE_MISSING` 回到全局其他。

新增 `purpose_placement.validate_placement_purpose`，首次落位及明确重新整理共用：

1. 从候选的唯一用途清单引用读取持久化包，候选的来源字符串本身不能作为授权。
2. 校验包 workspace、root、taxonomy、目标分类、授权来源和请求引用；检查清单摘要存在且新证据中的摘要与持久化记录一致。旧证据未保存摘要时只允许回读原有不可变记录，不从候选补造清单。
3. 校验工作副本当前 DocumentVersion、Document ID 和哈希；沿同一 managed_file 的当前 READY 源修订查找冻结成员版本。源分析版本与工作副本版本 ID 不同是正常血缘，不要求二者 ID 相等。
4. 复用 `evaluate_purpose_package` 核验成员版本、managed_file_id、SHA-256 及当前受控分类。工作副本编辑、源修订变化、成员不在清单、跨 workspace/root 或 taxonomy 过期均不能继承。
5. 把独立的运行时验证结果传入 `AutoPlacementPolicy.evaluate`。只有验证成功且对应同一个包和分类时，清单证据才可替代成员自身正文证据。风险检查和解析状态门槛继续有效，不伪造页码。

证据新增 `manifest_digest`，随建议 evidence_json 保留。没有运行时验证的包候选返回 `PURPOSE_PLACEMENT_INVALID` 并收纳其他。原有 ToolInvocation、ChangeSet、正式 PRIMARY 和路径审计流程不变。

### 19.2 完整正文部门兜底

`apply_unclassified_fallback` 不再无条件复用早先 `rule_fallback`，每次基于当前证据重算。`_build_scoped_fallback` 独立遍历 `fallback_policy.department_category_ids` 的受控节点，不要求部门父节点进入业务 Top-N；部门词仍来自该节点的名称、别名和正向信号。

正文唯一确定组织及部门时可从根级其他细化到部门其他；存在严格文号才进入发文。多个部门竞争时不保留旧部门归属。普通文件名、目录、此前的候选归属不能代替当前正文证据。已成立具体业务分类的展示仍不追加兜底辅助标签。

### 19.3 人才、考核与人员管理

| 正文题名或结构 | 目标 |
| --- | --- |
| CJ/人才计划人员申报汇总、青年教师奖申报汇总，同时含姓名、申报项目/项目类别、人才计划/入校时间/职称等三组字段 | 学校/学院—人事师资—人才工作 |
| 校招共用，同时含引才用才/人才目录/人才需求与编制/引进/申报/聘任等动作 | 学校/学院—人事师资—人才工作 |
| 人事代理合同期满考核评分表或考核情况汇总，同时含评价目标/考核结果、评价标准/考核结论、得分/姓名/岗位三组字段 | 学校/学院—人事师资—考核聘任 |
| 人员表首部同时含聘任时间、聘期、任职状态、单位名称、工号/姓名五组字段 | 学校/学院—人事师资 |
| 拟调入教师/教师调动/人员调动与提前进校/调动材料/调入申请同时成立 | 学校/学院—人事师资 |

新增对象动作规则启用 `require_subject_and_action_in_body`：两组均须在正文成立，不能用文件名补足。结构题名必须来自原正文前 600 字，字段来自前 5000 字；仅匹配时允许字符换行归一，quote 仍来自原文。字段规则不降低全局分数门槛。

强结构候选优先保留在有限候选集内，然后由既有主类选择器校正学校/学院镜像，避免长表履历中的教学、科研、信息化、职称等背景词将尚未校正的结构候选挤出。新增结构同步作为材料包锚点；包完整性、2–50 成员限制及冲突拒绝机制不变。

### 19.4 验证及生效边界

新增 `test_classification_v20_regressions.py` 覆盖完整分类服务到落位策略、真实 SQLite 持久化用途包、源与工作版本桥接、哈希/版本/workspace/taxonomy/摘要/分类失配、无运行时验证的伪包、部门缺席 Top-N、旧兜底冲突、文件名诱导和长人员表。

真实正文/用途包只读回放的九个固定样本结果：专业建设方案进入学院教学，高层次人才申报书进入学校人才工作，人事处临时通知进入学校人事师资其他，校招共用通知进入学校人才工作，CJ 与霍英东汇总表进入学院人才工作，合同期满考核评分表进入学校考核聘任，外聘兼职教师采集表及提前进校申请进入学院人事师资。这是定向问题回归，不代表全库准确率或最终其他比例。外聘长表的最终结果同时通过自动落位策略检查。

最终定向回归 164 项全部通过（121.87 秒），覆盖 matcher、规则策略、用途仓库/推断/落位、完整分类服务、新鲜度、主类选择、分类移动/组织查询及 managed worker；包括新增长流程“扫描—解析—持久化用途刷新—正式 PRIMARY—真实业务路径发布”，以及明确重新整理不二次确认。规则 JSON Schema、全部规则目标稳定 ID 校验和 `git diff --check` 均通过。未运行与本次无关的前端构建或全项目测试。

真实文件验证使用数据库只读事务，复用完整 DocumentPage，在内存生成新结果，不创建分类运行、不搬动工作副本。重启相关 API/worker 后才加载 v20；源侧刷新可复用解析。已经 ACTIVE 的工作副本须经现有受控重新分类/整理流程更新正式关系与物理位置，不能用一次纯分类刷新宣称目录已经更正。本轮不重置数据库或执行全量物理重分类。
