...
上传归档属于后台生命周期任务，原有实现只在浏览器内维护 `uploadBatch` 展示状态，刷新后无法恢复。现改为：

- 后端新增按会话读取上传归档业务状态的受控接口，不返回 `SYSTEM_AUDIT` 消息或物理路径。
- 前端启动聊天页时恢复已启动的上传归档状态，并继续轮询尚未完成的条目。
- 上传归档回执复用 WorkBuddy 文档流和分类树，不再单独显示旧式大进度卡。
- 已被普通任务回执覆盖的文件不重复渲染，避免同一文件在刷新后出现两份结果。
