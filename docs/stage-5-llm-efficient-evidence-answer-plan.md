...
18. 没有完整文件名或显式附件时，模糊召回只能生成候选选择卡；用户可选择一份或多份，选择后必须
    续跑原始总结、表格、分类或问答意图，不能统一改写成证据问答。
19. 普通概览总结优先复用当前活动版本的 `DocumentSummary` 及其可验证原文引用；摘要缺失、过期、
    引用不足，或用户明确要求完整、详细、逐章、逐页、精确事实、数字、日期和表格时才读取正文。
    多份文件确认后先读取各自已有摘要，再根据本轮要求汇总，不默认重新生成逐文件摘要。
...
6. 同名但内容不同、同一表达命中多个活动工作副本、或用户指代无法唯一落到稳定 ID 时，必须创建持久化
   歧义上下文并展示文件选择卡。用户可以选择一份或多份；选择前不得把候选一起总结、比较或计算。
