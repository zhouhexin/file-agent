...
实施结果：正文抽取、受管源分析、工作副本解析、上传/受管重命名分析、表格 Profile、统计分析和质量校验均已统一通过 `ReadableDocumentSourceResolver` 消费版本级派生件；生产代码不再调用临时 `prepared_spreadsheet_path()`。对应正式迁移为 `20260826_0001`。
