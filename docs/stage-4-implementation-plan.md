...
LLM 结构化检索计划采用相同的无界文件级召回规则。以“学校的工作总结”为例，模型只能把
“工作总结”声明为不可拆分的核心短语；“学校”默认表示当前学校工作区，校级是排序偏好而非
字面必需词。只有用户明确要求“只找校级”时才应用校级硬过滤。候选显示字段按每批 100 份
富化；摘要回退必须保留完整短语准入条件，并把结果标记为可能相关。
