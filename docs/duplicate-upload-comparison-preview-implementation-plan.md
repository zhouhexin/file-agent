...
> Excel 预览不再继续扩展制表符文本方案；结构化工作簿预览及后端单元格事实的后续设计见 [Excel 结构化预览实施方案](./structured-excel-preview-implementation-plan.md)。

- 状态：阶段一最小实现（2026-08-27）
- 日期：2026-08-26
...
- 新上传查重排除 `TRASHED`，继续比较共享 `ACTIVE` 工作副本和没有工作副本的当前受管文件。
- 没有工作副本的 TXT、MD、CSV、TSV 当前文件也参与本地 token 近似比较。
- 前端复用现有文件内容、受管文件预览和正文预览接口，提供双栏“本次上传 / 现有文件”查看。
- 图片、PDF 和文本直接预览；不超过 20 MB 的 DOCX 使用 `docx-preview` 在浏览器沙箱中本地渲染，
  不允许外部资源访问。DOC、DOCM、超限 DOCX 以及其他 Office 文件已有 `document_pages` 时展示正文，
  否则明确降级为下载后查看。
- 对比弹窗内保留“取消本次上传”“继续上传并独立保留”“使用现有文件”决策；尚未物化的受管文件不显示“使用现有文件”。
- 桌面对比弹窗默认使用可用视口高度，支持拖动右下角自定义大小和一键最大化；预览栏按窗口宽度在
  双栏与上下排列之间自适应，DOCX 页面宽高不再固定为 Word 原始纸张尺寸。

本次不新增 comparison API、数据库字段、异步 `PREPARE_DUPLICATE_PREVIEW` 任务、服务端 Office 上传侧按需解析、
候选版本快照或确定性差异摘要。这些继续作为后续增强项，不属于本次精简实现完成条件。
