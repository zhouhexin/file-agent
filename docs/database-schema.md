...
解析运行通过可空 `document_version_id` 固化正文所属内容版本。迁移前历史数据保持 NULL；单文档存在多个
版本时，未绑定版本的历史解析结果不得用于新建 Chunk，避免把旧正文错误绑定到新版本。

```sql
create table document_extraction_runs (
  id uuid primary key default gen_random_uuid(),
  document_id uuid not null references documents(id),
  document_version_id uuid null references document_versions(id) on delete cascade,
  ...
  parser_config_hash varchar(64) not null default '',
  error_message text null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
### 4.12 document_index_runs 与 document_chunks
...
create table document_index_runs (
  ...
  tokenizer varchar(40) not null default 'jieba',
  tokenizer_version varchar(80) not null default '',
  config_hash varchar(64) not null,
  status varchar(40) not null,
  chunk_count integer not null default 0,
  evidence_count integer not null default 0,
  embedding_status varchar(40) not null default 'DISABLED',
  error_code varchar(100) null,
  error_message text null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (document_version_id, extraction_run_id, config_hash)
);

create table document_chunks (
  id uuid primary key default gen_random_uuid(),
  index_run_id uuid not null references document_index_runs(id) on delete cascade,
  document_id uuid not null references documents(id) on delete cascade,
  document_version_id uuid not null references document_versions(id) on delete cascade,
  extraction_run_id uuid not null references document_extraction_runs(id) on delete cascade,
  ...
  sheet_name varchar(255) null,
  cell_range varchar(80) null,
  element_ids_json jsonb not null default '[]'::jsonb,
  embedding vector(1536) null,
  embedding_status varchar(40) not null default 'DISABLED',
  embedding_provider varchar(80) not null default 'disabled',
...
create index ix_document_chunks_search_text_trgm on document_chunks using gin(search_text gin_trgm_ops);
create index ix_document_versions_filename_trgm on document_versions using gin(filename gin_trgm_ops);
```

阶段三默认由应用层 Jieba 生成空格分隔 `search_text`，PostgreSQL 使用 `simple` 配置构建
...
create table answer_references (
  id uuid primary key default gen_random_uuid(),
  qa_answer_id uuid not null references qa_answers(id) on delete cascade,
