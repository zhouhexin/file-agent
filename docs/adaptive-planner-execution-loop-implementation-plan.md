...
文件检索的“找到”只表示发现候选，不能自动等价于“该文件可以支持用户的事实结论”。所有入口必须遵守
同一套分级和授权规则：

```text
用户查询
-> LLM（可选）理解目标、选择 Catalog 中的 Tool、提出条件说明
-> 后端解析核心主题、范围、时间与关系词
-> 检索 Tool 召回候选
-> 后端验证当前版本原文证据
   -> SUPPORTED：可展示为“已验证相关”，可绑定给 evidence-answer
   -> POSSIBLE：只命中文件名、分类、摘要、泛动作词或部分主题，只能供用户查看
-> evidence-answer 只读取 SUPPORTED 或用户明确选定的文件当前版本
```

具体规则：

1. LLM 只能给出受控查询、`interpreted_conditions` 和后续 Tool 建议；不得把“相关”“可能涉及”
   直接写成文件事实，也不得把候选 `document_id` 自行升级为证据范围。
2. 对“涉及/包含/提到/出现”等原文关系查询，若同时存在核心业务主题和宽泛动作词，后端必须验证两者
   均出现在同一活动工作副本的当前版本正文中，才标为 `SUPPORTED`。
3. 仅命中“发放、管理、工作、安排、统计”等宽泛动作词，或仅有文件名、分类、摘要线索时，结果必须标为
   `POSSIBLE`，前端需明确显示“可能相关”，不得显示为已验证事实。
4. 正文索引不可用而降级为摘要检索时，原文关系查询的结果全部是 `POSSIBLE`；不得因降级而放宽为
   `SUPPORTED`。
5. 聊天主入口、`/api/search` 兼容接口、异步检索续跑、LangGraph `search -> observe -> replan`、
   全局 evidence-answer 候选召回必须复用此边界。`POSSIBLE` 结果不得出现在 Tool 绑定的
   `document_ids` 中，也不得成为 Adaptive Planner 后续读取文件的授权来源。
6. 用户明确选择候选文件后，系统仍必须重新校验该文件的 ACTIVE 状态和当前版本；选择仅确定范围，
   不能替代回答结论所需的原文 EvidenceSpan 校验。

这条规则的目的是消除“检索列表里有文件，但回答阶段错误声称相关、或把泛词命中当成结论”的系统性问题，
而不是为某个具体业务词添加特殊答案。
