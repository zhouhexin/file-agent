...
- READY 项进入前端可勾选 OperationPlan，用户最终选中的文件才允许执行。
- NEEDS_REVIEW 项保留在持久化重命名批次中，通过后续对话提供名称后独立确认执行，不阻止 READY 项。
...
3. `NEEDS_REVIEW` 项不进入自动建议 OperationPlan，也不阻止 READY 项；用户通过对话提供名称后独立执行。
