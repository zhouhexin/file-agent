...
- 如果相同 SHA-256 只命中已经移入回收站的文件，卡片必须提示“此前已删除，是否再次上传”；
  选择“再次上传”后按新文件建立工作副本，原回收站记录不能被自动恢复或合并。
...
7. 再次移入回收站后，输入完整文件名，例如：`查找《待删除通知.txt》`。
8. 页面必须提示该文件已删除并展示恢复单选卡；只输入 `查找有关通知的文件` 时不得搜索回收站。
9. 准备两条同名、同版本的回收站记录后再次按完整文件名查找，确认页面逐条展示“文件 1、文件 2”，
