...
用户上传文件并输入明确任务文字后，系统必须自动把文件整理成可通过对话访问的工作副本。当前阶段
有附件时也不允许空文字提交，不得用系统默认任务猜测用户意图。允许归档的文件完成查重和
...
首次导入只能生成标准化命名建议，不能自动改变工作副本文件名。无论建议置信度如何，工作副本、
DocumentVersion 和受管原件在首次提交时都必须保留用户上传时的文件名；回执可展示建议名称、依据和
“当前尚未改名”的说明。只有用户后续在对话中明确提出改名时，系统才能基于工作副本重新生成
`RENAME_WORKING_COPIES` OperationPlan；用户确认该计划后才执行真实改名。命名建议不是用户授权，
不得因为首次导入发生物理重命名。
