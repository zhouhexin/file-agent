...
首次标准化命名发生在工作副本首次对用户可见之前，属于创建工作副本的内部整理步骤，不是修改已经
存在的活动文件，因此不要求逐文件生成 OperationPlan。该步骤仍必须通过受控服务执行并记录证据和
审计；原始文件不得被改名、移动、覆盖或删除。
...
上传完成后的首次标准化命名遵守 3.6：只在隐藏临时工作副本中执行，并在原子提交为 `ACTIVE` 前
完成，不生成针对活动文件的 OperationPlan。首次整理后的任何再次改名都必须以 `working_copy_id` 为
目标生成 `RENAME_WORKING_COPIES` OperationPlan。自动建议发生目标名称冲突时不得直接生成后缀或覆盖，
必须先通过对话询问用户是否同时保留；只有用户确认同时保留后，后端才可以在扩展名前生成稳定版本后缀，
并同时校验文件系统、工作副本索引和同批目标。
