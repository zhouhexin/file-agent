...
首次命名与既有活动工作副本发生同名冲突时不得自动添加版本后缀，也不得覆盖。系统必须通过对话逐文件
询问用户是否同时保留：确认同时保留后才分配“第二版、第三版”等稳定后缀；不同时保留时应提示用户选择
保留已有工作副本、用新文件替换现有工作副本或删除现有工作副本。替换、覆盖或删除只允许作用于工作副本，
必须经过 OperationPlan；不可变受管原件仍不得覆盖或删除。
...
`working_copy_id` 为目标生成 `RENAME_WORKING_COPIES` OperationPlan。建议目标名称发生冲突时不得直接
生成后缀或覆盖，必须先通过对话询问用户是否同时保留；只有用户确认同时保留后，后端才可以在扩展名前
生成稳定版本后缀，并同时校验文件系统、工作副本索引和同批目标。
