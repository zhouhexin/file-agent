...
对应后台摘要 Provider 设置为 `llm` 后才允许调用模型。用户在对话中明确要求总结、讲解或针对正文
问答时，可以继续使用独立的聊天文档摘要 LLM Provider，并必须读取完整 `document_pages` 正文、遵守
分块上限和缓存边界。抽取式摘要与 LLM 摘要都只能用于概览、召回和分类候选，不能替代原文证据。
