...
- 新策略无法可靠确定业务主类、组织范围或正文质量不足时，必须完成归档并把唯一 PRIMARY 设置为
  `system.other`（显示和物理目录均为“其他”）；不得为新版分类生成“待分类”“待复核”节点、队列或
  阻塞状态。历史 `.other`、`.issued` 和分类 `NEEDS_REVIEW` 仅兼容读取，不再参与新候选召回。
