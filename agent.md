...
首次命名与既有活动工作副本发生同名冲突时不得自动添加版本后缀，也不得自动覆盖。系统必须通过对话
逐文件展示冲突并询问用户选择覆盖已有文件、同时保留或取消。当前会话只有一个有效冲突时，用户明确回复
“是”“覆盖”或“覆盖原文件”即构成覆盖确认，后端在创建并确认 OperationPlan、保存 Confirmation、
ChangeSet 和旧状态审计后直接执行，不得再要求第二次重复确认。确认同时保留后才分配“第二版、第三版”
等稳定后缀，并继续沿用现有冲突计划确认闭环。短回复无法唯一对应冲突时必须先让用户选择具体文件。
替换、覆盖或删除只允许作用于工作副本；不可变受管原件仍不得覆盖或删除。
