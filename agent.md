...
用户要求输出详细实现方案、技术方案或开发计划时，必须同时把完整方案写入项目 `docs/`
目录中的独立 Markdown 文档；对话回复只能作为摘要和文档链接，不能成为详细方案的唯一载体。
后续实现发生重要设计调整时，必须同步更新对应方案文档。

遇到冲突时按以下顺序判断：

1. 用户在当前对话中的明确指令。
2. 本文件 `agent.md`。
3. `docs/automatic-organization-conversational-access-implementation-plan.md`，作为当前阶段自动整理和对话访问的直接实施依据。
4. `/Users/zhouhexin/Downloads/conversational_file_agent_implementation_plan_v1.md`，作为产品定位和长期架构参考。
5. `docs/conversational-file-agent-development-blueprint.md`。
6. `docs/superpowers/plans/2026-06-24-file-agent-mvp-implementation-plan.md`。
...
本项目后续计划中的“低耗模式”只允许表示降低 LLM 的调用成本，包括减少模型调用次数、限制输入输出
token、只发送经过检索的少量证据、优先使用轻量模型、使用安全缓存和限制失败重试。不得把“低耗”
解释为关闭或削弱文件扫描、导入、解析、OCR、分类、Chunk、Evidence、检索、LangGraph、Tool、
ChangeSet、OperationPlan、日志或审计能力，也不得把当前服务器没有 GPU 当成产品功能限制。

LLM 关闭、超时或失败时，File Agent 的确定性文件能力必须继续工作；证据回答可以返回已验证证据或
“未找到明确依据”，但不能为了节省模型调用而输出猜测性结论。未来任何使用“低耗”表述的开发计划
都必须明确它约束的是 LLM 调用预算，而不是项目运行能力。
...
### 3.5 普通用户不感知 Skill 和 Tool

普通用户使用 File Agent 时只需要上传文件和用自然语言提出需求，不需要理解 Skill、Tool、AgentRun、
ChangeSet、解析器、向量索引或内部状态机。上述对象必须继续作为系统内部编排、安全和审计边界存在，
但不得成为普通用户完成任务所需学习的概念。

普通用户界面和消息回执只展示：

- 正在处理、已经整理、需要确认、部分失败等用户任务状态。
- 整理后的工作副本文件名。
- 多标签分类建议、年份、文档类型、关键词、实体和可定位证据。
- 搜索推荐原因、原文引用和可继续执行的自然语言操作。
- 原始文件是否保持不变，以及确实需要用户决策的冲突或低置信度事项。

普通用户界面不得展示：

- Skill 名称、Tool 名称、LangGraph 节点和 Planner 内部计划。
- ToolInvocation、Graph State、数据库表名和内部队列名。
- 服务器绝对路径、密钥、Prompt、模型内部响应和不必要的处理状态。

admin/ops 可以在受权限控制的审计页面查看必要的 AgentRun、ToolInvocation、ChangeSet、模型版本和
失败诊断信息。后端必须为普通用户建立独立的用户任务回执投影，不能把内部运行状态或 Tool 输出直接
透传给前端后再依赖前端隐藏。

### 3.6 首次上传自动整理，后续高风险变更确认

聊天页中，用户上传文件并输入明确任务文字后，系统必须自动把文件整理成可通过对话访问的工作副本；
只有附件而没有文字时仍不允许提交。WorkBuddy 本地批量导入和会话附件导入属于独立结构化入口：用户
指定授权目录并调用 `file_batch_ingest`，或在 WorkBuddy 消息中明确提交附件并由宿主调用
`workbuddy_attachment_ingest` 后，固定目录或附件提交事件与 `AUTO_ORGANIZE` 策略就是明确任务，允许
`user_request=null` 自动分类、命名、落位和索引，不再要求额外聊天文字。会话附件只能来自预配置的
WorkBuddy 缓存根，MCP 必须固定附件 ID、文件名、大小、mtime 和哈希，后端不得接收宿主绝对路径。
允许归档的文件完成查重和
基础格式/MIME/宏/加密风险检查后，由异步 worker 在隐藏临时工作副本中依次完成解析/OCR、普通摘要、分类主题摘要、年份/
文号/标题/单位/关键词/实体提取、多标签分类建议、首次标准化命名和检索索引构建，最后原子提交为
`ACTIVE` 工作副本。

外部受管目录自动导入必须复用同一首次落位边界：先由 `SOURCE_ANALYSIS` 在只读原件上完成并持久化
正文解析、摘要、索引和 taxonomy 全分类建议，再由 `MATERIALIZE_WORKING_COPY` 创建隐藏
`ORGANIZING` 工作副本并复用这些结果。通过统一置信度、正文证据和候选间隔门槛后，工作副本以原
文件名首次发布到 taxonomy `organization_path`；未通过门槛时发布到不暴露源分类目录结构的内部
中性路径，并记录为 `ACTIVE + NEEDS_REVIEW`。复核队列是逻辑查询投影，不得阻止文件检索、读取、
预览、下载或对话操作。已经成为 `ACTIVE` 的文件不得借此规则静默移动，后续移动仍必须经过
OperationPlan 和用户确认。

WorkBuddy MCP 对已入库文件的搜索必须固定调用只读搜索接口；文件读取只允许受控的读取、总结或讲解
模式，不能接受可能路由到文件写操作的任意 instruction；证据问答不接受客户端自造 Evidence。明确
重命名必须携带稳定 Document ID、当前文件名和目标文件名，并复用现有受控重命名与 OperationPlan 审计，
不能由 MCP 直接修改文件。刷新或重连后必须从后端恢复澄清项和 OperationPlan 状态。

### 3.7 唯一共享工作目录与开发重置

所有用户的可操作文件必须落在唯一的系统共享工作区和唯一物理工作目录中：外部受管原始资料与用户
上传归档各自只导入一份，不得按用户、默认工作区、历史会话或扫描次数重复复制。用户的 default
workspace 仍只用于登录后的会话、上传来源、反馈与审计；它不是物理文件副本范围。工作副本、检索投影、
回收站和高风险 OperationPlan 必须使用稳定的 `SYSTEM_SHARED` 工作区与 `shared/<root_key>` 存储前缀。

物理共享不等于共享个人数据：用户上传暂存、来源会话、反馈和确认记录仍按用户校验；但工作副本不区分
创建用户，所有登录用户都可以检索、读取、下载、引用和选择 `SYSTEM_SHARED` 工作区中的 `ACTIVE`
工作副本。重复上传命中活动共享工作副本时，必须提供“使用现有文件”选项，并可展示该共享工作副本当前
文件名和逻辑路径；不得展示最初上传用户、上传暂存路径、来源会话或个人审计信息。所有共享文件变更都
必须在 OperationPlan 中明确提示“作用于所有用户共用的文件工作目录”，并由发起用户确认；受管原始目录
仍然只读。上传查重范围包含共享目录中的 `ACTIVE` 工作副本，以及当前仍在受管文件索引中、尚未同步或
物化成工作副本的当前文件；只排除 `TRASHED` 工作副本及其对应的已删除文件。回收站不得参与精确哈希、
同名或近似内容匹配，只在用户明确按完整文件名查找恢复时进入独立恢复选择流程。

需要重建开发环境时，必须使用受控重置命令并显式确认。重置只能清空数据库业务表、共享工作副本、共享
回收站、上传暂存和 `MANAGED_ROOT_ARCHIVE_WRITE_PATH` 中的旧上传归档原件；必须保留外部
`MANAGED_ROOT_*` 受管原始资料目录和 `alembic_version`，并在路径重叠或目标不安全时停止。
...
`LLM_ENABLED=true` 只表示对话理解等已授权能力可用，不能隐式开启后台文件摘要外发；只有部署显式把
对应后台摘要 Provider 设置为 `llm` 后才允许调用模型。用户在对话中提出普通概览总结时，必须先复用
当前活动 `DocumentVersion` 已完成的持久化摘要及其原文引用；只有摘要缺失、过期、不足以回答，或用户
明确要求完整、详细、逐章、逐页、精确事实、数字、日期、表格和证据时，才读取 `document_pages`、
Chunk 或 EvidenceSpan 正文。多份文件经用户选择后也先读取各文件已有摘要，再按本轮目标汇总，不得
默认重新生成每份摘要。抽取式摘要与 LLM 摘要不能独立充当事实证据，最终结论仍必须回到可定位原文引用。

“文种”可以作为分类、检索和证据解释使用的结构化元数据，但当前阶段不得作为独立字段加入建议文件名。
如果正文原始标题本身包含“通知”“报告”“纪要”等词，应把它作为标题内容正常保留；不得在标题之外
再次追加文种，也不得为了移除文种而改写原始标题含义。

WorkBuddy 本地批量导入在固定目录、固定清单和冻结 `AUTO_ORGANIZE` 策略范围内，首次归档允许对新建
工作副本自动应用通过证据校验的标准化名称；这次授权只覆盖首次整理，不需要额外 OperationPlan。
命名依据不足时必须保留原名并记录 `NO_CHANGE`，不算任务失败或部分失败。不可变归档原件始终保留
上传时的文件名和字节，不得随工作副本自动命名而变化。聊天附件入口及不带上述冻结授权的其他首次
导入继续遵守各自入口规则，不能借 WorkBuddy 策略扩大授权。工作副本成为 `ACTIVE` 后，只有用户明确
提出改名，系统才能生成 `RENAME_WORKING_COPIES` OperationPlan，并在用户确认后执行真实改名。

工作副本成为 `ACTIVE` 后，再次重命名、移动、复制、覆盖、删除、批量导出或外部发送都属于高风险
物理副作用，必须先生成 OperationPlan 并由用户确认。分类建议、摘要和逻辑检索标签不等于物理目录
移动；系统应优先使用稳定 `working_copy_id`、整理后的文件名、多标签分类、摘要、实体和原文索引支持
对话访问，不得要求普通用户理解目录层级。

分类落位有两项窄范围例外，优先于后文的一般 OperationPlan 确认规则：

- 新策略无法可靠确定具体业务主类时，必须完成归档，不得生成“待分类”“待复核”节点、队列或阻塞状态。
  如果正文可以可靠确定学校/学院组织范围及部门业务域，则使用“学校/学院—部门—发文/其他”收纳：
  有可定位正式文号时进入该部门“发文”，否则进入该部门“其他”；这些 `FALLBACK` 节点不参加普通业务
  候选竞争。组织范围或部门也无法可靠确定时，才把唯一 PRIMARY 设置为 `system.other`（显示和物理目录
  均为“其他”）。分类 `NEEDS_REVIEW` 仍仅兼容读取，不参与新候选召回。
- 用户通过聊天、结构化 UI/API 或 MCP 明确提交 `SET_PRIMARY` 或 `MOVE` 时，该提交本身就是本次移动的
  明确授权。后端完成身份、稳定对象 ID、当前版本、revision、taxonomy、受控目标和冲突校验后直接执行，
  不再要求第二次确认；仍须创建服务端授权快照、内部 OperationPlan、逐文件 placement operation、
  ToolInvocation、ChangeSet 和路径审计。对象或目标有歧义时先请求选择，选择完成后继续同一操作且不追加
  确认。该例外不扩展到删除、覆盖、复制、独立重命名、恢复、外发或其他高风险动作。

低置信度命名或分类不得阻塞同批其他文件。低置信度命名继续沿用用户上传时的原文件名，并把该文件标记
为待确认；原文件名只在该文件的确认上下文中展示。用户后续通过自然语言更正文件名或分类时，后端必须
精确解析到稳定工作副本 ID；分类纠正进入反馈和候选关系，活动工作副本改名仍须经过 OperationPlan。

后台同步必须允许不同目录和不同来源中的同名文件全部正常导入，不得把文件写入用户可见的“待确认”
物理目录，也不得在同步期主动要求用户处理同名冲突。同名事实保留在工作副本索引中，并只在三类时机
提示：用户上传同名文件、自然语言查询命中多个同名候选、用户实际读取或操作相关文件。提示必须列出
有权限候选并要求用户按稳定文件 ID 选择；选择前不得合并正文、自动覆盖或自动添加版本后缀。用户随后
明确提出覆盖、同时保留、改名、移动或删除时，仍必须创建并确认 OperationPlan，保存 Confirmation、
ChangeSet 和路径审计。替换、覆盖或删除只允许作用于工作副本；不可变受管原件仍不得覆盖或删除。
...
路径、目录名、文件名、扩展名和文件元数据只能作为候选召回、弱信号和初始命名字段，不能单独作为最终分类依据。最终分类必须通过受控解析后的正文、OCR 文本、PDF 页文本、Word 段落、Excel 工作表/单元格、压缩包内子文件清单等内容证据确认；如果文件名与正文证据冲突，以正文和可定位证据为准。对 `通知`、`工作安排`、`审批表`、`会议纪要`、`日报表`、`制度汇编`、压缩包和扫描件等泛化文件名，必须读取内容后再确认业务主题、日期、涉及单位和重命名建议；文种可以独立识别为分类或检索元数据，但当前阶段不得作为独立字段加入建议文件名。

上述文件名限制对“部门—发文”有一项窄范围例外：文件名命中严格正式文号格式，且文号中的发文机关
前缀能唯一映射到 taxonomy `fallback_policy.department_category_ids` 中的受控学校/学院部门时，可以直接
生成该部门 `.issued` 范围 fallback。该例外只能证明组织、部门和“发文”，不能证明职称、招聘、财务事项
等具体业务叶类；近似编号、普通机构关键词或多个部门竞争时不得应用。

主类选择必须先过滤到达到当前证据门槛、具有原正文页码或 Sheet 定位证据、且没有明确负向冲突的业务候选，
再按“业务分＋受限标题主题加成＋受限正文开头加成”排序。跨业务分支第一名与第二名差值小于 0.10 时，
只要第一名的最终排序分严格更高，仍以第一名作为唯一 PRIMARY；只有最终排序分完全并列、证据相互矛盾或
确实是多主题汇编时才归“其他”。正文已明确学校/学院范围但候选位于相反组织镜像时，必须切换到 taxonomy
中同一业务的学校/学院镜像节点；没有受控镜像时不得编造路径。

当前常见材料强结构规则至少覆盖：`师资引进申请报告 -> 人才工作`、人才项目“推荐意见 -> 人才工作”、
`教师职务聘期任务书 -> 师资招聘`、`西安理工大学教职工离职审批表 -> 学校/人事师资`。这些规则仍须
同时使用正文主旨或字段结构生成可定位证据，普通“报告、意见、任务书、审批表”词不得单独决定分类。材料包
用途继承必须识别“高层次人才特殊支持计划/三秦英才特殊支持计划＋科技创新领军人才/青年拔尖人才申报书”
等真实题名变体；分类版本刷新也必须重新尝试构建不可变用途包，不能只在首次解析时尝试一次。
...
当前文件物理副作用只允许作用于工作副本。受管原始目录对 Agent、用户接口和普通 Tool 永远只读，
历史 `RENAME_FILES`、`RENAME_UPLOADED_FILES`、Native/F2 原地重命名通道不得再接入新的 Planner 或
OperationPlan。活动工作副本统一使用 `RENAME_WORKING_COPIES`、`MOVE_WORKING_COPIES`、
`TRASH_WORKING_COPIES` 和 `RESTORE_WORKING_COPIES`；确认后必须由真实 `confirmed-file-action`
白名单执行器完成物理动作，并写入 ToolInvocation、ChangeSet、逐文件 ChangeItem 和必要的路径/回收站
记录。没有白名单执行器的 OperationPlan 确认必须失败并保持未执行状态，绝不能只修改状态或返回虚假的
`EXECUTED`。

上传完成后的首次命名只产生回执建议，隐藏临时工作副本和最终 `ACTIVE` 工作副本均保留上传文件名；
不得把命名建议当成首次导入参数，也不得创建未经用户请求的重命名计划。用户明确提出改名后，才以
`working_copy_id` 为目标生成 `RENAME_WORKING_COPIES` OperationPlan。建议目标名称发生冲突时不得自动
生成后缀或自动覆盖，必须先通过对话询问用户覆盖已有文件、同时保留或取消。当前唯一冲突下用户明确
...
既有基础能力继续按方案 B 演进；当前阶段必须优先执行
`docs/automatic-organization-conversational-access-implementation-plan.md`，不得先扩展普通用户不感知的
图谱、自动 Skill 演化或新的高风险物理动作。当前阶段顺序为：

```text
1. 修复 confirmed-file-action 虚假执行结果，保持高风险操作安全边界
2. 固化上传、查重、归档、隐藏导入、自动命名、分类和 ACTIVE 工作副本闭环
3. 建立面向普通用户的 UserTaskReceipt 投影，隐藏 Skill/Tool 内部概念
4. 实现 DocumentVersion 级 Chunk、Evidence、全文索引和 embedding
5. 实现最终文件名、分类、摘要到原文 Chunk 的两阶段混合检索
6. 实现 evidence-answer、引用持久化和无依据回答
7. 实现文件名与分类的自然语言纠正边界
8. 完成后端、前端和端到端智能文件任务验证
```
...
- 上传文件格式必须受控；文件大小使用部署级可配置资源上限并采用流式写入，不能用过小的固定业务阈值拒绝正常学校材料，也不能允许单请求无限占用内存、磁盘或解析时间。
- 不执行上传文件中的宏、脚本、链接或嵌入对象。
- 普通用户可以访问唯一共享工作目录中的 `ACTIVE` 工作副本；只能访问自己的 default workspace
  会话、个人附件上下文、上传来源、反馈和操作确认记录。共享物理文件访问不得扩大为跨用户个人数据访问。
