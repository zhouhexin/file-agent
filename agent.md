...
只有附件而没有文字时仍不允许提交。WorkBuddy 本地批量导入属于独立结构化入口：用户指定授权目录并
调用 `file_batch_ingest` 后，固定目录与 `AUTO_ORGANIZE` 策略就是明确任务，允许
`user_request=null` 自动分类、命名、落位和索引，不再要求额外聊天文字。允许归档的文件完成查重和
