...
文件重命名必须额外遵守批次完整性边界：对话解析出的单文件范围必须由后端按完整相对路径或稳定文件 ID 精确确认；目录范围必须先固化为持久化重命名批次。批次内存在 `NEEDS_REVIEW` 文件时不得只为 READY 子集创建部分 OperationPlan。只有每个文件都已自动命名、由用户提供名称或被用户明确排除后，才能为全部可执行文件创建一个不可变 OperationPlan。前端可以分页展示批次，但不得因为只展示前若干项而缩小真实执行范围。
