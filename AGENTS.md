...
聊天页中仅选择附件时，仍必须由用户输入明确任务文字后才能提交，不允许空文字直接发送附件。
WorkBuddy 本地批量导入是独立的结构化任务入口：用户先指定已授权目录并调用 `file_batch_ingest`，
即构成按冻结 `AUTO_ORGANIZE` 策略自动分类、命名、落位和索引的明确授权，`user_request=null`
不需要再补一条聊天文字。WorkBuddy 会话附件同样只能由宿主传入本轮稳定提交 ID、附件 ID、原文件名
和位于预配置缓存根内的真实文件引用，并显式调用 `workbuddy_attachment_ingest`；该调用构成固定附件清单的
提交授权，允许 `user_request=null` 自动整理。普通聊天页、WorkBuddy 本地目录和 WorkBuddy 会话附件三种
入口不得互相放宽；后端必须以固定目录或附件提交事件、不可变清单和冻结策略审计授权范围。
该授权允许首次归档对新建工作副本应用证据充分的标准化名称；证据不足时保留原名且不算失败。
不可变归档原件不得改名，工作副本首次发布后的任何再次改名仍必须经过 OperationPlan 和用户确认。

WorkBuddy 对已入库文件的搜索、读取和证据问答必须使用独立只读 MCP Tool：搜索文字不得进入文件写操作
路由，读取只允许固定只读模式，证据只能由 File Agent 从完整原文生成和校验。明确重命名 MCP Tool 必须
同时固定稳定 Document ID、当前文件名和目标文件名，并复用现有受控重命名、OperationPlan、冲突检查和
审计链路；MCP 不得直接访问或修改工作副本。
...
上传、导入和分类阶段的普通文档摘要与分类主题摘要默认使用本地 CPU-only 抽取式 Provider，采用
Jieba 分词和有候选上限的 LexRank 句子排序，不调用外部模型、不要求 GPU。`LLM_ENABLED=true` 不能
隐式开启后台文件摘要外发；只有显式把对应摘要 Provider 设置为 `llm` 后才允许调用模型。用户明确
要求总结或讲解时仍可使用独立的聊天文档摘要 LLM Provider，但最终证据必须回到完整原文。
...
文件重命名必须额外遵守批次选择边界：对话解析出的单文件范围必须由后端按完整相对路径或稳定文件 ID 精确确认；目录范围必须先固化为持久化重命名批次。`READY` 文件可以独立生成 OperationPlan，`NEEDS_REVIEW` 文件单独列为待确认项，不得阻止其他文件确认执行。前端必须用复选框让用户确定本次执行集合，默认选中自动建议项；取消勾选的文件不得进入最终执行计划。待确认文件继续通过“文件原文件名更正为新文件名”的对话方式独立确认和执行，单项失败、冲突或歧义不得影响其他文件。前端可以分页展示批次，但后端必须校验选择项属于当前用户和当前计划。
