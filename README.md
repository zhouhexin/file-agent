...
- `docs/2026-09-10-workdata-classification-rule-optimization-plan.md`、`docs/2026-09-10-classification-optimization-development-spec.md`：分类 v10、唯一 OTHER、直接落位授权和分阶段验收的当前依据。
- `docs/classification-evaluation/README.md`：受控离线分类评测输入、输出和运行方式。
...
图片/扫描 PDF 的表格结构抽取可通过 `STRUCTURED_EXTRACTION_LAYOUT_PROVIDER=tencent_cloud_table` 切换到腾讯云 `RecognizeTableAccurateOCR`；该能力复用腾讯云凭证和外发授权，返回可定位的单元格、行列范围和坐标，字段映射与证据校验仍由结构化抽取服务完成。
文件重命名统一生成 `RENAME_WORKING_COPIES` OperationPlan，确认后由工作副本执行器执行；旧的受管原始文件 Native/F2 执行通道和上传暂存重命名通道不再对 Agent 开放。
上传附件通过查重后由独立 worker 归档到受管原始目录；安全默认配置下，`IMPORT` worker 以一次源文件读取完成哈希和原子复制，按 `shared/<root_key>/<源相对路径>` 登记 `ACTIVE` 工作副本，随后由 `ANALYSIS` worker 异步完成正文解析、双摘要、分类和 Chunk 索引。新版分类使用 taxonomy v10：正文证据不足、组织范围不明或解析不完整时仍完成归档，并将唯一 PRIMARY 投影为 `system.other`（显示和物理目录均为“其他”），不创建新的分类“待复核/待分类”队列或目录。`CLASSIFICATION_QUALITY_MODE=shadow` 只记录观察结果；未发布校准产物时只能使用 `conservative_rules`，不得把历史 top1 分数称为已校准概率。用户在聊天、UI/API 或 MCP 中明确提交 `SET_PRIMARY` 或 `MOVE` 后，经对象、版本、revision、taxonomy 和冲突校验即创建带真实授权快照的内部操作并执行，不再要求第二次确认；删除、覆盖、独立重命名、恢复和外发仍保留原确认边界。已经 `ACTIVE` 的文件绝不因规则升级静默迁移。普通受管目录在启动扫描后先由 `SOURCE_ANALYSIS` 建立只读正文与证据索引，每个修订 READY 后自动创建低优先级 `MATERIALIZE` 任务，最终把全部文件同步到共享工作目录。同步未完成期间，检索合并活动工作副本和未物化源侧索引；用户命中的既有物化任务会被提升优先级，同一文件不并发复制。同步不会因不同目录中的同名文件停顿，也不会生成“待整理/待确认”物理目录；同名歧义只在上传、查询或实际使用相关文件时提示选择。后台双摘要默认使用 CPU-only Jieba + LexRank。普通用户不展示内部状态、Skill 或 Tool。对话找文件默认使用 CPU-only 两阶段检索，精确问答仍必须回到原文取证。后续重命名、移动和删除计划必须以 `working_copy_id` 为对象，不能再修改受管原始目录。
...
分类判定默认仍为 `LLM_CLASSIFICATION_MODE=rule_only`。如需让 LLM 在候选分类内做语义判定，可设置 `LLM_CLASSIFICATION_MODE=hybrid`；如需允许 LLM 自由提出新分类路径，还必须显式设置 `LLM_CLASSIFICATION_ALLOW_FREE_PATHS=true`。自由路径仅作为内部诊断建议，不能写入正式 taxonomy、正式 `document_categories` 或用户可见的分类复核队列；当前 taxonomy 无法可靠细分时仍使用 `system.other`。
